...
function UAD_integrate_prepare_display_context(&$output, &$message)
{
	...
	$ua_browser = array(
		...
		2 => (($message['ua_browser_icon']) ? '<img src="'.$settings['default_images_url'].'/user_agent_display/icons/browsers/'.$message['ua_browser_icon'].'.png" align="top" alt="" title="'.htmlspecialchars($message['user_agent']).'" /> ' : '') . $message['ua_browser'] . ' <img src="' . $settings['default_images_url'] . '/user_agent_display/icons/operatingsystems/' . $message['ua_os_icon'] . '.png" align="top" alt="' . $message['ua_os'] . '" title="' . htmlspecialchars($message['user_agent']) . '" /> ' . $message['ua_os'] . ' • ' . htmlspecialchars($message['user_agent']),
